...
The `Insecticides` config param is a list of dictionaries, one per insecticide. For each insecticide, genotype-specific modifications of killing, blocking, repelling, and larval killing can be set. In this example, all *A. gambiae* vectors with an allele combination of "bb" will be completely resistant to pyrethroid.
