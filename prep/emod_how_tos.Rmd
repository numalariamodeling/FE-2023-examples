...
This csv is passed as the `site_file` to `emodpy-malaria's` `generate_weather()` function along with the `node_column` that provides the name of our node ID column in the csv and `local_dir` which specifies that output directory. From there we can also specify:
...
inputs folder. To reference these when running a simulation, update the configuration parameters to reflect the name and location of your climate files:
...
One can also specify different simulation types such as **VECTOR_SIM** to simulate just the vector model without the malaria within-host model, or other simulation types listed [here](https://docs.idmod.org/projects/emod-malaria/en/latest/glossary.html?highlight=Sim_Type#term-simulation-type). After setting the team config defaults, reset the simulation type wtih `config.parameters.Simulation_Type = "MALARIA_SIM"`.
...
EMOD configuration scripts contain five primary components:
    ...
    2. Config setup & simulation duration
        - Set `config.json` parameters to team defaults and update additional parameters as needed using a callback to the `emod-api` config. For example, one can specify different simulation types such as **VECTOR_SIM** to simulate just the vector model without the malaria within-host model, or other simulation types listed [here](https://docs.idmod.org/projects/emod-malaria/en/latest/glossary.html?highlight=Sim_Type#term-simulation-type).
    ...
    3. Campaign setup
        - Build a campaign file using the `emod-api` schema. This is where desired interventions should be added.
          ...
          ```
    4. Demographics
        - Build a demographics file using `emod-api`. This is typically done through `from_template_node` for single node sims, but there are other methods available if needed (*see demographics how to*)
...
The example below would populate the model with 100% gambiae mosquitoes and can be included in the config builder.
...
This parameter will scale all habitat parameters for the entire simulation duration without changing the temporal dynamics, so that a new transmission is achieved with the same ratios among the species and same time profile. For example, setting `x_Temporary_Larval_Habitat` 
...
In this example, we show how to change the `Simulation_Duratiion` and `Run_Number` parameters. Simulation duration is set in days, and in this example is set to last 5 years (5 yrs * 365 days/yr).
